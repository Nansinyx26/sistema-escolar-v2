...
function startKeepAlive(customUrl) {
    ...
    if (process.env.NODE_ENV !== 'production') {
        logger.debug('[KeepAlive] Desativado em desenvolvimento');
        return;
    }
    ...
    if (!targetUrl) {
        logger.warn('[KeepAlive] Nenhuma URL configurada — cold start não será prevenido');
        return;
    }
    ...
    const ping = () => {
        ...
        req.on('error', (err) => {
            logger.warn(`[KeepAlive] Ping falhou`, { error: err.message });
        });

        req.setTimeout(10000, () => {
            req.destroy();
            logger.warn('[KeepAlive] Ping timeout (10s)');
        });
    };
    ...
    logger.info(`🔄 [KeepAlive] Ativo`, { targetUrl, intervalMin: 14 });
}
