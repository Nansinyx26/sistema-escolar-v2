...
exports.create = async (req, res) => {
    try {
        ...
        const doc = await Especial.create({
            ...req.body,
            // Depois do spread: o `escolaId` do contexto vence o que vier no
            // corpo, senão o cliente escolheria a escola do registro.
...
exports.get = async (req, res) => {
    try {
        const doc = await Especial.findById(req.params.id);
...
exports.update = async (req, res) => {
    try {
        const doc = await Especial.findByIdAndUpdate(req.params.id, req.body, { new: true });
        res.json({ success: !!doc, data: doc });
    } catch (e) {
        res.status(400).json({ success: false, error: e.message });
    }
...
exports.delete = async (req, res) => {
    try {
        await Especial.findByIdAndDelete(req.params.id);
