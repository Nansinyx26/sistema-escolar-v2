...
exports.update = async (req, res) => {
    try {
        ...
        if (callingUserPerfil !== 'admin' && String(targetId) !== String(callingUserId)) {
            return res.status(403).json({ success: false, error: 'Acesso negado. Você só pode atualizar sua própria conta.' });
        }
...
exports.updateTutorial = async (req, res) => {
    const userId = req.user.id || req.user._id;
    const { tutorialProfessorConcluido, tutorialResponsavelConcluido, reiniciar } = req.body;

    try {
        const update = {};
        if (reiniciar) {
            if (req.user.perfil === 'professor' || req.user.perfil === 'admin' || req.user.perfil === 'diretor') {
                update.tutorialProfessorConcluido = false;
            }
            if (req.user.perfil === 'responsavel') {
                update.tutorialResponsavelConcluido = false;
            }
        } else {
            if (tutorialProfessorConcluido !== undefined) {
                update.tutorialProfessorConcluido = !!tutorialProfessorConcluido;
                if (tutorialProfessorConcluido) update.tutorialProfessorConcluidoEm = new Date();
            }
            if (tutorialResponsavelConcluido !== undefined) {
                update.tutorialResponsavelConcluido = !!tutorialResponsavelConcluido;
                if (tutorialResponsavelConcluido) update.tutorialResponsavelConcluidoEm = new Date();
            }
        }

        const user = await Usuario.findByIdAndUpdate(userId, { $set: update }, { new: true }).lean();
        if (!user) return res.status(404).json({ success: false, error: 'Usuário não encontrado.' });
