...
function gerarPdf(conversa, nomeUsuario) {
    ...
    const conteudo = [
        ...
        {
            text: `Exportado em ${dataLegivel(new Date())}`,
            fontSize: 8,
            color: CINZA,
            margin: [0, 0, 0, 16],
        },
...
function paragrafo(texto, { negrito = false, tamanho = 22, cor = null } = {}) {
    const runs = String(texto)
        .split('\n')
        .map((linha, i) => {
            const quebra = i > 0 ? '<w:br/>' : '';
            return (
                `<w:r><w:rPr>${negrito ? '<w:b/>' : ''}<w:sz w:val="${tamanho}"/>` +
...
function documentoXml(conversa, nomeUsuario) {
    ...
    if (conversa.resumo) {
        corpo.push(
            paragrafo('Resumo dos trechos mais antigos', {
                negrito: true,
                tamanho: 18,
                cor: '6B7280',
            })
    ...
    for (const m of conversa.mensagens || []) {
        corpo.push(
            paragrafo(`${autorDe(m.papel, nomeUsuario)} — ${dataLegivel(m.em)}`, {
                negrito: true,
                tamanho: 16,
                cor: m.papel === 'usuario' ? '111827' : '10B981',
            })
        );
...
function gerarDocx(conversa, nomeUsuario) {
    return montarZip([
        ...
        { nome: '_rels/.rels', dados: Buffer.from(RELS_XML, 'utf8') },
        {
            ...
            dados: Buffer.from(documentoXml(conversa, nomeUsuario), 'utf8'),
        },
    ]);
}
