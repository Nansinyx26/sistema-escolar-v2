...
class VoiceService {
    ...
    async generateInsightText(prompt, opts = {}) {
        ...
        if (typeof prompt === 'string' && prompt.length > MAX_PROMPT_CHARS) {
            logger.warn(
                `[VoiceService] Prompt truncado de ${prompt.length} para ${MAX_PROMPT_CHARS} chars.`
            );
        ...
        for (const mod of modelos) {
            ...
            console.log(
                `[VoiceService] Chamando Gemini (${mod}) para: "${prompt.substring(0, 50)}..."`
            );
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    lastError = err;
                    logger.warn(
                        `[VoiceService] Erro no modelo ${mod}: ${errMsg}. Tentando próximo fallback...`
                    );
                    continue;
                }
