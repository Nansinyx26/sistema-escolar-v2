...
| Consentimento colhido no cadastro, **nunca presumido** | **Pronto** | `register-docente`, `-diretor`, `-secretaria` e `register-code` recusam (400) sem o aceite no corpo; no `register-responsavel` ele é opcional, porque o portal só abre depois do `CompletarCadastro`, que o pede (Issue #288). Quando vem, as cinco rotas o gravam no `lgpdHistory` com `metodoValidacao: FORMULARIO_CADASTRO` ([`services/conformidade/consentimentoCadastro.js`](../backend/src/services/conformidade/consentimentoCadastro.js)). Até a Issue #236 a conta nascia com `consentimentoAceiteEm` carimbado sem ninguém marcar nada; a migração `1788652800000-invalidar-consentimento-carimbado-no-cadastro` tira o carimbo das contas antigas. Gestor que cria conta de outra pessoa não consegue gravar consentimento por ela (Issue #295) |
...
| Separação das portas de login (escola x família) | **Pronto** | [`utils/portaisDeLogin.js`](../backend/src/utils/portaisDeLogin.js) — cada página de login declara a porta dela e o servidor recusa perfil de outro público **depois** de conferida a senha, para a recusa não virar oráculo de enumeração |
