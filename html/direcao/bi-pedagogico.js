...
async function initBI() {
    ...
    try {
        ...
        try {
            ...
            if (salva && salva.id) {
                urlParams = `?escolaId=${salva.id}`;
            }
        } catch(e) {}

        // Carregamento paralelo para melhor performance
        const [heatmapRes, insightsRes, chartDataRes] = await Promise.all([
            fetch(`${API_BASE}/ia/mapa-calor${urlParams}`, { signal, credentials: 'include' }).catch(e => { console.error('[BI] Fetch mapa-calor falhou:', e.message); return null; }),
            fetch(`${API_BASE}/ia/insights-global${urlParams}`, { signal, credentials: 'include' }).catch(e => { console.warn('[BI] Fetch insights-global falhou:', e.message); return null; }),
            fetch(`${API_BASE}/dashboard/chart-data${urlParams}`, { signal, credentials: 'include' }).catch(e => { console.warn('[BI] Fetch chart-data falhou:', e.message); return null; })
        ]);
