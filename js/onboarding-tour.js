...
(function () {
    ...
    const addTourStyles = () => {
        ...
        style.textContent = `
            ...
            @media (max-width: 768px) {
                ...
                #onboarding-popup h3 { font-size: 1.1rem !important; }
                #onboarding-popup p { line-height: 1.4 !important; margin-bottom: 1rem !important; }
                .tour-glass-btn { padding: 8px 14px; font-size: 0.75rem; }
            }
        `;
    ...
    function createUI() {
        ...
        // Container do orb flutuante do tour (canto inferior direito)
    ...
    function positionPopup(target) {
        ...
        if (isMobile()) {
            ...
            if (spaceBelow > popupH + 60) {
                ...
                top = rect.top - popupH - 20;
            } else {
                // If neither fits, center vertically too
        ...
        const triggerTourSpeak = () => {
            ...
            const onStarted = () => {
                if (window.VoiceOrbManager) {
                    const orbEl = document.getElementById('tour-orb-fab');
                    if (orbEl) {
                        window.VoiceOrbManager.init(orbEl, { mode: 'fab', voiceName: 'Adam' });
                        window.VoiceOrbManager.setState('speaking');
                    }
                }
            };
            // Para o orb quando o áudio termina
            const onEnded = () => {
                if (window.VoiceOrbManager) {
                    window.VoiceOrbManager.setState('idle');
                    setTimeout(() => window.VoiceOrbManager.destroy(), 1500);
                }
            };
        ...
        try {
            ...
            if (data.success && data.user) {
                ...
                if (data.user.perfil === 'responsavel') {
                    tourSteps = RESPONSAVEL_STEPS;
                } else if (data.user.perfil === 'admin' || data.user.perfil === 'diretor') {
                    tourSteps = DIRECTOR_STEPS;
                } else {
                    tourSteps = PROFESSOR_STEPS;
                }
                currentStep = 0;
                createUI();
                renderStep();
