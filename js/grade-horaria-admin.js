document.addEventListener('DOMContentLoaded', async () => {
    ...
    const loadSelects = async () => {
        // Show loading state on selects
        selProf.innerHTML = '<option value="">⏳ Carregando professores...</option>';
        selTurma.innerHTML = '<option value="">⏳ Carregando turmas...</option>';
        filterProf.innerHTML = '<option value="">⏳ Carregando...</option>';
        ...
        tabelaBody.innerHTML = `
            <tr>
                <td colspan="6" style="text-align: center; padding: 3rem;">
                    <div class="grade-loader-wrapper" style="justify-content: center; font-size: 1rem;">
                        <span class="grade-loader"></span> Carregando horários da grade...
                    </div>
                </td>
            </tr>
        `;
    ...
    const renderTable = (lista) => {
        ...
        if (lista.length === 0) {
            tabelaBody.innerHTML = `
                <tr>
                    <td colspan="6" style="text-align: center; padding: 3rem;">
                        <i class="bi bi-calendar-x" style="font-size: 2.5rem; color: var(--primary); display: block; margin-bottom: 0.75rem;"></i>
                        <strong>Nenhum horário cadastrado.</strong>
                        <p style="color: var(--text-secondary); margin-top: 0.25rem; font-size: 0.9rem;">Use o formulário acima para registrar novos horários.</p>
                    </td>
                </tr>
            `;
            return;
