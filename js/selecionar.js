...
const SelecionarPage = {
    ...
    updateUI(user) {
        // Atualiza o nome em todos os locais possíveis
        const els = {
            header: document.getElementById('userName'),
            fixed: document.getElementById('userNameFixed'),
            avatar: document.getElementById('userAvatarSelecionar')
        };

        if (els.header) els.header.textContent = user.nome;
        if (els.fixed) els.fixed.textContent = user.nome;

        if (els.avatar && user.nome) {
            const inicial = user.nome.charAt(0).toUpperCase();
            els.avatar.innerHTML = user.foto ?
                `<img src="${user.foto}" style="width:100%; height:100%; object-fit:cover; border-radius:50%;">` :
                `<span>${inicial}</span>`;
        }
            ...
            return `
                <div class="professor-row" style="display:flex; align-items:center; gap:10px; margin-top:8px; padding:6px; background:rgba(255,255,255,0.03); border-radius:8px; border:1px solid rgba(255,255,255,0.05);">
                    <div class="foto-mini" style="width:30px; height:30px; border-radius:50%; background:var(--bg-elevated); overflow:hidden; display:flex; align-items:center; justify-content:center; border:1px solid var(--border-secondary); flex-shrink:0;">
                        ${p.foto ? `<img src="${p.foto}" style="width:100%; height:100%; object-fit:cover;">` : `<span style="font-size:12px; color:var(--primary); font-weight:bold;">${inicial}</span>`}
                    </div>
