...
(function (root, factory) {
    ...
    function normalizeUrl(url) {
        ...
        try {
            var parser = typeof URL !== 'undefined' && url.includes('://') ? new URL(url) : null;
            if (parser) {
    ...
    function isDashboardUrl(url) {
        ...
        for (var i = 0; i < DASHBOARDS.length; i++) {
            ...
            if (clean === dash || clean === dash.replace('/index.html', '/') || clean === dash.replace('.html', '')) {
                return true;
            }
        }
        // Identificação por caminhos específicos
        if (clean === '/html/direcao' || clean === '/html/direcao/' || clean === '/html/dashboard') {
            return true;
        }
        return false;
    }
    ...
    function montarInterface() {
        ...
        if (isDash) {
            ...
            var dashBtns = document.querySelectorAll('.btn-voltar-global, #btnVoltarNav, [data-voltar], a.btn-voltar');
