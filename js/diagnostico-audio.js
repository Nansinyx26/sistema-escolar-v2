...
function initDiagnostics() {
    ...
    const updateStatus = async () => {
        ...
        try {
            ...
            if (!data.status) throw new Error(data.error || 'Resposta inesperada do servidor');

            const { gemini, elevenlabs } = data.status;

            // Gemini
            applyProviderStatus('gemini', gemini);
            if (gemini.connectionOk) {
                addLog('✅ Google Gemini TTS: online e funcionando!', 'success');
            } else if (gemini.configured) {
                addLog(`⚠️ Google Gemini TTS: chave configurada mas conexão falhou — ${gemini.lastError}`, 'warn');
            } else {
                addLog('❌ Google Gemini TTS: GOOGLE_TTS_API_KEY não encontrada no backend/.env', 'error');
            }

            // ElevenLabs
            applyProviderStatus('eleven', elevenlabs);
            if (elevenlabs.connectionOk) {
                addLog('✅ ElevenLabs: online e autenticado!', 'success');
            } else if (elevenlabs.configured) {
                addLog(`⚠️ ElevenLabs: chave configurada mas conexão falhou — ${elevenlabs.lastError}`, 'warn');
            } else {
                addLog('ℹ️ ElevenLabs: ELEVENLABS_API_KEY não configurada (opcional — Gemini é o principal)', 'info');
            }

            // Resumo final
            const anyOk = gemini.connectionOk || elevenlabs.connectionOk;
            if (anyOk) {
                addLog('✅ Sistema de voz operacional. Ao menos um provedor está ativo.', 'success');
            } else {
                addLog('❌ Nenhum provedor de voz disponível. Configure GOOGLE_TTS_API_KEY no backend/.env', 'error');
            }

        } catch (err) {
            addLog(`❌ Falha na verificação: ${err.message}`, 'error');
            // Mostra erro nos dois painéis
            ['gemini', 'eleven'].forEach(prefix => {
    ...
    const testSpeak = async () => {
        ...
        const gender   = document.getElementById('test-gender')?.value || 'female';
