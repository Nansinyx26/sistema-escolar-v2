...
(function () {
  ...
  function textoUltimoAcesso(iso) {
    ...
    return dia === 'Hoje' ? `visto hoje às ${hora}`
      : dia === 'Ontem' ? `visto ontem às ${hora}`
  ...
  function iniciais(nome) {
    const letras = String(nome || '?').trim().split(/\s+/).slice(0, 2)
  ...
  function comprimirImagem(file) {
    return new Promise((resolve) => {
      ...
      if (!tipo.startsWith('image/') || tipo === 'image/gif' || file.size <= COMPRIMIR_ACIMA_DE) {
        resolve(file);
        return;
      }
      ...
      img.onload = () => {
        try {
          ...
          canvas.toBlob((blob) => {
            ...
            const nome = String(file.name || 'imagem').replace(/\.[^.]+$/, '') + '.jpg';
            resolve(new File([blob], nome, { type: 'image/jpeg' }));
          }, 'image/jpeg', IMG_QUALIDADE);
        } catch (e) {
          URL.revokeObjectURL(url);
          resolve(file);
        }
  ...
  const EMOJI_LIST = ['😀','😃','😄','😁','😆','😅','😂','🤣','😊','😇','🙂','🙃','😉','😌','😍','🥰','😘','😗','😙','😚','😋','😛','😝','😜','🤪','🤨','🧐','🤓','😎','🤩','🥳','😏','😒','😞','😔','😟','😕','🙁','☹️','😣','😖','😫','😩','🥺','😢','😭','😤','😠','😡','🤬','🤯','😳','🥵','🥶','😱','😨','😰','😥','😓','🤗','🤔','🤭','🤫','🤥','😶','😐','😑','😬','🙄','😯','😦','😧','😮','😲','🥱','😴','🤤','😪','😵','🤐','🥴','🤢','🤮','🤧','😷','🤒','🤕','🤑','🤠','😈','👿','👹','👺','🤡','💩','👻','💀','☠️','👽','👾','🤖','🎃','😺','😸','😹','😻','😼','😽','🙀','😿','😾','👋','🤚','🖐','✋','🖖','👌','🤏','✌️','🤞','🤟','🤘','🤙','👈','👉','👆','🖕','👇','☝️','👍','👎','✊','👊','🤛','🤜','👏','🙌','👐','🤲','🤝','🙏','✍️','💅','🤳','💪','🦾','🦵','🦿','🦶','👂','🦻','👃','🧠','🦷','🦴','👀','👁','👅','👄','💋','🩸','❤️','🧡','💛','💚','💙','💜','🖤','🤍','🤎','💔','❣️','💕','💞','💓','💗','💖','💘','💝','💟','☮️','✝️','☪️','🕉','☸️','✡️','🔯','🕎','☯️','☦️','🛐','⛎','♈️','♉️','♊️','♋️','♌️','♍️','♎️','♏️','♐️','♑️','♒️','♓️','🆔','⚛️','🉑','☢️','☣️','📴','📳','🈶','🈚️','🈸','🈺','🈷️','✴️','🆚','💮','🉐','㊙️','㊗️','🈴','🈵','🈲','🅰️','🅱️','🆎','🆏','🅾️','💦','🔥','✨','🎉','🎊','🚀','⭐','🌟','💥'];
  ...
  class ChatWindowInstance {
    ...
    q(prefixo) { return this.el.querySelector(`#${prefixo}_${this.targetUserId}`); }

    bindEvents() {
      const w = this.el;

      this.q('btnBack')?.addEventListener('click', (e) => { e.stopPropagation(); this.close(); });
      this.q('btnMinimize').addEventListener('click', (e) => { e.stopPropagation(); this.toggleMinimize(); });
      this.q('chatHeaderUser').addEventListener('click', () => { if (this.isMinimized) this.toggleMinimize(); });
      this.q('btnClose').addEventListener('click', (e) => { e.stopPropagation(); this.close(); });
      ...
      this.q('btnCloseSearch').addEventListener('click', () => { searchBar.hidden = true; this.limparBusca(); });
      ...
      this.q('btnSelForward').addEventListener('click', () => this.manager.abrirModalEncaminhar([...this.selecionadas]));
      ...
      this.q('btnDiscardPreview').addEventListener('click', () => this.descartarPreviaAudio());
    ...
    close() {
      ...
      this.pendingFiles.forEach((p) => p.url && URL.revokeObjectURL(p.url));
      this.el.remove();
    ...
    async atualizarPresenca() {
      try {
        const res = await fetch(`${API()}/chat-direto/presenca/${this.targetUserId}`, { credentials: 'include' });
        if (!res.ok) return;
        ...
        this.atualizarStatusHeader();
      } catch (e) { /* mantém o status recebido do card */ }
    }
    ...
    atualizarStatusHeader() {
      ...
      const rotulo = status === 'online' ? '🟢 Online'
    ...
    onPresenca(status, ultimoAcesso) {
      ...
      else if (status === 'offline') this.targetUserData.ultimoAcesso = new Date().toISOString();
      this.atualizarStatusHeader();
    }

    onTyping(isTyping) {
      const statusEl = this.q('chatStatus');
      if (!isTyping) { this.atualizarStatusHeader(); return; }
    ...
    onRecording(isRecording) {
      const statusEl = this.q('chatStatus');
      if (!isRecording) { this.atualizarStatusHeader(); return; }
      statusEl.className = 'chat-header-status typing';
    ...
    emitTyping() {
      ...
      window.socket.emit('chat:typing', { destinatarioId: this.targetUserId, isTyping: true });
      clearTimeout(this.typingTimer);
      this.typingTimer = setTimeout(() => {
        window.socket.emit('chat:typing', { destinatarioId: this.targetUserId, isTyping: false });
      }, 2000);
    }
    ...
    async carregarMaisAntigas() {
      ...
      if (sentinela) { sentinela.textContent = 'Carregando…'; sentinela.disabled = true; }
      ...
      try {
        ...
        if (antigas.length > 0) {
          ...
          this.messages = antigas.filter((m) => !conhecidas.has(String(m._id))).concat(this.messages);
          this.renderMessages();
        ...
        if (s) { s.textContent = '↑ Ver mensagens anteriores'; s.disabled = false; }
      } finally { this.carregandoMais = false; }
    }
    ...
    async enviarLeitura() {
      ...
      const temNaoLidas = this.messages.some((m) => String(m.destinatarioId) === meuId && !m.lida);
      if (!temNaoLidas) return;

      this.messages.forEach((m) => {
        if (String(m.destinatarioId) === meuId) { m.lida = true; m.status = 'lida'; }
      });
      ...
      } catch (e) { /* nova tentativa na próxima abertura */ }
    }
    ...
    renderSingleBubble(m) {
      ...
      const ticks = isSent ? `<span class="chat-msg-ticks ${isRead ? 'read' : ''}" title="${isRead ? 'Lida' : 'Enviada'}">${isRead ? '✓✓' : '✓'}</span>` : '';
      ...
      if (m.respostaParaId) {
        ...
        if (ref) {
          const autor = String(ref.remetenteId) === meuId ? 'Você' : (this.targetUserData.nome || 'Contato');
      ...
      if (m.apagadaParaTodos) {
        ...
        if (m.mensagem) contentHtml += `<div class="chat-msg-texto">${esc(m.mensagem)}</div>`;
      ...
      if (Array.isArray(m.reacoes) && m.reacoes.length > 0) {
        ...
        m.reacoes.forEach((r) => {
          ...
          quem[r.emoji] = (quem[r.emoji] ? quem[r.emoji] + ', ' : '') + (r.usuarioNome || 'Usuário');
        });
        reacoesHtml = `<div class="chat-reacoes-list">${Object.keys(counts).map((emoji) =>
    ...
    bindBubbleEvents(escopo) {
      ...
      raiz.querySelectorAll('.btn-audio-play').forEach((btn) => {
        btn.addEventListener('click', () => {
          ...
          if (this._audioAtual && !this._audioAtual.paused) {
            ...
            this._audioBtn && (this._audioBtn.innerHTML = `<i class="bi bi-play-fill"></i>`);
            if (this._audioBtn === btn) { this._audioAtual = null; this._audioBtn = null; return; }
          }
          ...
          audio.play().catch(() => { btn.innerHTML = `<i class="bi bi-play-fill"></i>`; });
          audio.ontimeupdate = () => {
      ...
      raiz.querySelectorAll('.chat-reacao-badge').forEach((badge) => {
        badge.addEventListener('click', () => this.sendReaction(badge.dataset.msgId, badge.dataset.emoji));
      });

      // Citação leva até a mensagem original
      raiz.querySelectorAll('.chat-reply-quote[data-goto]').forEach((quote) => {
        quote.addEventListener('click', () => {
          const alvo = this.bodyEl.querySelector(`#msgRow_${CSS.escape(quote.dataset.goto)}`);
          if (!alvo) return;
    ...
    showMsgActionMenu(msgId, targetEl) {
      ...
      const largura = 190;
      menu.style.left = Math.max(8, Math.min(rect.left - 40, window.innerWidth - largura - 8)) + 'px';
      const alturaMenu = menu.offsetHeight;
      menu.style.top = (rect.bottom + alturaMenu > window.innerHeight
      ...
      menu.querySelectorAll('.emoji-opt').forEach((opt) => {
        opt.addEventListener('click', () => { this.sendReaction(msgId, opt.dataset.emoji); fechar(); });
      });
      ...
      menu.querySelectorAll('.menu-opt').forEach((opt) => {
        opt.addEventListener('click', () => {
          ...
          else if (acao === 'encaminhar') this.manager.abrirModalEncaminhar([String(msg._id)]);
    ...
    async copiarTexto(texto) {
      ...
      try {
        ...
        ta.select();
        try { document.execCommand('copy'); this.manager.toast('Texto copiado.'); }
        catch (err) { this.manager.toast('Não foi possível copiar.'); }
        ta.remove();
      }
    }
    ...
    copiarSelecionadas() {
      const textos = this.messages
        ...
        .map((m) => `[${formatTime(m.createdAt || m.dataEnvio)}] ${String(m.remetenteId) === this.manager.meuId ? 'Você' : (this.targetUserData.nome || 'Contato')}: ${m.mensagem}`);
      if (textos.length === 0) { this.manager.toast('Nenhum texto nas mensagens selecionadas.'); return; }
    ...
    async apagarSelecionadas() {
      ...
      for (const id of ids) {
        try {
          await fetch(`${API()}/chat-direto/mensagem/${id}?tipo=para_mim`, { method: 'DELETE', credentials: 'include' });
        } catch (e) { /* segue para as demais */ }
      }
      this.sairModoSelecao();
      this.loadHistory();
    }
    ...
    async deleteMsg(msgId, tipo) {
      ...
      if (!confirm(paraTodos ? 'Apagar esta mensagem para todos?' : 'Apagar esta mensagem para você?')) return;
      try {
      ...
      } catch (e) { this.manager.toast('Erro ao apagar mensagem.'); }
    }
    ...
    async sendReaction(msgId, emoji) {
      ...
      const msg = this.messages.find((m) => String(m._id) === String(msgId));
      const minha = msg && (msg.reacoes || []).find((r) => String(r.usuarioId) === this.manager.meuId);
      ...
      } catch (e) { /* o evento de socket ainda pode chegar */ }
    }
    ...
    async sendMessage() {
      ...
      if (this.editingMsgId) {
        ...
        } catch (e) { this.manager.toast('Erro ao editar mensagem.'); }
        this.cancelarEdicao();
        return;
      }
    ...
    renderPendingFiles() {
      ...
      box.hidden = false;
      box.innerHTML = this.pendingFiles.map((p, i) => {
    ...
    encerrarCaptura() {
      ...
      this.isRecording = false;
      window.socket?.emit('chat:recording', { destinatarioId: this.targetUserId, isRecording: false });
    }
    ...
    tocarPreviaAudio() {
      ...
      btn.innerHTML = `<i class="bi bi-pause-fill"></i>`;
      this._previewAudio.play().catch(() => { btn.innerHTML = `<i class="bi bi-play-fill"></i>`; });
      this._previewAudio.onended = () => { btn.innerHTML = `<i class="bi bi-play-fill"></i>`; };
    }

    descartarPreviaAudio() {
      if (this._previewAudio) { this._previewAudio.pause(); this._previewAudio = null; }
      if (this.audioPreview) { URL.revokeObjectURL(this.audioPreview.url); this.audioPreview = null; }
      this.audioChunks = [];
    ...
    appendMessage(msg) {
      ...
      const naBase = this.bodyEl.scrollHeight - this.bodyEl.scrollTop - this.bodyEl.clientHeight < 120;
    ...
    anexarBolha(msg, anterior) {
      ...
      if (!anterior) { this.renderMessages(); return; }
      ...
      fragmento.innerHTML =
        (dia && dia !== diaAnterior ? `<div class="chat-day-divider"><span>${esc(dia)}</span></div>` : '') +
        this.renderSingleBubble(msg);

      const novos = [...fragmento.children];
      novos.forEach((el) => this.bodyEl.appendChild(el));
      // Só o que acabou de entrar recebe listeners.
      novos.forEach((el) => this.bindBubbleEvents(el));
    }
  ...
  class ChatManager {
    ...
    openChat(targetUserId, targetUserData) {
      ...
      if (window.matchMedia('(max-width: 768px)').matches) {
        this.activeWindows.forEach((w) => w.close());
      }
    ...
    sincronizarIconeSom() {
      ...
      this.toast(somAtivo() ? 'Som das notificações ativado.' : 'Som das notificações desativado.');
    }
    ...
    notificarMensagem(msg) {
      ...
      const previa = msg.mensagem
        ...
        : (msg.audio && msg.audio.url) ? '🎤 Mensagem de voz'
        : (msg.anexo && msg.anexo.nome) ? `📎 ${msg.anexo.nome}` : 'Enviou um anexo';
      ...
      if (document.hidden && 'Notification' in window && Notification.permission === 'granted') {
        try {
          const n = new Notification(nome, { body: previa, tag: `chat-${msg.remetenteId}`, renotify: false });
          n.onclick = () => {
            window.focus();
    ...
    async abrirModalEncaminhar(mensagemIds) {
      ...
      try {
        const res = await fetch(`${API()}/professores/status-online`, { credentials: 'include' });
        const json = await res.json();
        const contatos = (json.data || []).filter((c) => String(c.userId) !== String(this.meuId));
        ...
        lista.innerHTML = contatos.map((c) => {
          ...
          return `
            <label class="chat-forward-item">
              ...
              ${foto ? `<img src="${esc(foto)}" alt="" class="chat-forward-avatar">`
        ...
        lista.querySelectorAll('input[type="checkbox"]').forEach((chk) => {
          chk.addEventListener('change', () => {
            if (chk.checked) escolhidos.add(chk.value); else escolhidos.delete(chk.value);
      ...
      btnEnviar.addEventListener('click', async () => {
        ...
        fechar();
        this.activeWindows.forEach((w) => w.sairModoSelecao && w.modoSelecao && w.sairModoSelecao());
      });
    }
    ...
    bindSocketEvents() {
      const checkSocket = () => {
        ...
        s.on('chat:mensagem', (msg) => {
          ...
          const outroId = souDestinatario ? String(msg.remetenteId) : String(msg.destinatarioId);
        ...
        s.on('chat:editada', (msg) => {
          const outroId = String(msg.remetenteId) === this.meuId ? String(msg.destinatarioId) : String(msg.remetenteId);
          this.activeWindows.get(outroId)?.substituirBolha(msg);
        });

        s.on('chat:apagada', ({ mensagemId, paraTodos }) => {
          this.activeWindows.forEach((w) => w.removerOuMarcarApagada(mensagemId, paraTodos));
        });

        s.on('chat:reacao', ({ mensagemId, reacoes }) => {
          this.activeWindows.forEach((w) => {
            const msg = w.messages.find((m) => String(m._id) === String(mensagemId));
            if (msg) { msg.reacoes = reacoes || []; w.substituirBolha(msg); }
          });
        });

        // Leitura: uma mensagem específica ou a conversa inteira
        s.on('chat:lida', ({ mensagemId, destinatarioId }) => {
          this.activeWindows.get(String(destinatarioId))?.marcarLidasLocalmente([mensagemId]);
        });

        s.on('chat:lidas', ({ mensagemIds, destinatarioId }) => {
          this.activeWindows.get(String(destinatarioId))?.marcarLidasLocalmente(mensagemIds);
        });

        // Presença do contato reflete direto no cabeçalho da conversa aberta
        s.on('presence:professor', ({ userId, online, status }) => {
          this.activeWindows.get(String(userId))?.onPresenca(status || (online ? 'online' : 'offline'));
        });
      };
