...
(function () {
    ...
    function criarModal() {
        ...
        modal.style.cssText =
            'position:fixed;inset:0;z-index:9999;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,.72);backdrop-filter:blur(6px);padding:16px;';
    ...
    function skeleton() {
        ...
        for (var i = 0; i < 4; i++) {
            html +=
                '<div style="height:58px;border-radius:10px;background:linear-gradient(90deg,rgba(255,255,255,.03),rgba(255,255,255,.07),rgba(255,255,255,.03));background-size:200% 100%;animation:mcaShimmer 1.2s infinite;margin:6px 4px;"></div>';
        }
        return (
            html +
            '<style>@keyframes mcaShimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}</style>'
        );
    }
    ...
    function carregar(q, sala) {
        ...
        var url =
            API_BASE + '/alunos/codigos-secretos' + (params.length ? '?' + params.join('&') : '');
        ...
        fetch(url, { credentials: 'include' })
            ...
            .catch(function (e) {
                if (minha !== requisicao) return;
                lista.innerHTML =
                    '<div style="text-align:center;color:#f87171;padding:24px;font-size:.85rem;"><i class="bi bi-exclamation-triangle"></i> ' +
    ...
    function render(alunos, filtros) {
        ...
        if (!alunos.length) {
            ...
            if (partes.length) {
                ...
                limpar.style.cssText =
                    'margin-top:12px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.14);border-radius:8px;color:#d4d4d8;padding:7px 14px;font-size:.8rem;font-family:inherit;cursor:pointer;';
        ...
        alunos.slice(0, 60).forEach(function (a) {
            ...
            row.style.cssText =
                'display:flex;align-items:center;gap:12px;padding:10px 10px;margin:4px;border-radius:10px;border:1px solid rgba(255,255,255,.06);background:rgba(255,255,255,.02);';
            ...
            nome.style.cssText =
                'color:#fff;font-size:.88rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;';
            ...
            var ondeEstuda =
                [a.ano && a.ano !== '-' ? a.ano : '', a.sala || a.turma || '']
                    .filter(Boolean)
                    .join(' · ') || 'Sem turma';
            meta.textContent =
                ondeEstuda + (a.vinculado ? ' · responsável vinculado' : ' · sem vínculo');
            meta.style.cssText =
                'color:' + (a.vinculado ? '#10b981' : '#94a3b8') + ';font-size:.72rem;';
            ...
            codigo.title = temCodigo
                ? ''
                : 'Não foi possível gerar o código deste aluno. Revise o cadastro e use o botão de gerar novo código.';
            ...
            btnRegen.addEventListener('click', function () {
                ...
                fetch(API_BASE + '/alunos/' + encodeURIComponent(a.id) + '/regenerar-codigo', {
                    ...
                    .catch(function (e) {
                        toast(e.message || 'Erro ao regenerar código.', 'error');
                    })
                    .finally(function () {
        ...
        if (!document.getElementById('mcaSpinStyle')) {
            ...
            st.id = 'mcaSpinStyle';
            st.textContent =
                '.mca-spin{display:inline-block;animation:mcaSpin 1s linear infinite}@keyframes mcaSpin{to{transform:rotate(360deg)}}';
    ...
    function botaoIcone(icone, titulo) {
        ...
        b.style.cssText =
            'background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:8px;color:#d4d4d8;width:34px;height:34px;cursor:pointer;font-size:.9rem;flex-shrink:0;transition:border-color .15s,color .15s;';
