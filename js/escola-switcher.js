...
(function () {
    ...
    // Cabeçalho dos painéis novos (épico #370): só o NOME da escola ativa.
    function preencherTopo(nome) {
        document.querySelectorAll('[data-escola-ativa-nome]').forEach(function (el) {
            el.textContent = nome;
        });
        document.querySelectorAll('[data-escola-ativa-topo]').forEach(function (el) {
            el.hidden = false;
        });
    }

    // Visual novo (body.ui3): mesma lógica, com classes de css/ui-painel.css
    // no lugar do estilo em linha.
    function renderUi3(perfil, ativa, outras) {
        var wrap = document.createElement('div');
        wrap.id = 'escolaSwitcherSidebar';
        wrap.className = 'pn-escola';

        var atual = document.createElement('button');
        atual.type = 'button';
        atual.className = 'pn-escola-atual';
        atual.innerHTML =
            '<i class="bi bi-building" aria-hidden="true"></i>' +
            '<span class="pn-escola-txt">' +
            '<span class="pn-escola-rot">Escola ativa</span>' +
            '<span class="pn-escola-nome" id="escolaAtivaNome"></span>' +
            '</span>' +
            (outras.length ? '<i class="bi bi-chevron-down" aria-hidden="true"></i>' : '');
        atual.querySelector('#escolaAtivaNome').textContent = ativa.nome;
        wrap.appendChild(atual);

        if (outras.length) {
            atual.setAttribute('aria-expanded', 'false');
            atual.setAttribute('aria-label', 'Escola ativa: ' + ativa.nome + '. Trocar de escola');
            var lista = document.createElement('div');
            lista.className = 'pn-escola-lista';
            lista.hidden = true;
            outras.forEach(function (e) {
                var btn = document.createElement('button');
                btn.type = 'button';
                btn.innerHTML = '<i class="bi bi-arrow-left-right" aria-hidden="true"></i>';
            ...
            atual.addEventListener('click', function () {
                ...
                atual.setAttribute('aria-expanded', String(!lista.hidden));
            });
        }
    ...
    function render(escolas, escolaAtivaId) {
        ...
        var ativa =
            escolas.find(function (e) {
                return String(e._id) === String(escolaAtivaId);
            }) || escolas[0];
        var outras = escolas.filter(function (e) {
            return String(e._id) !== String(ativa._id);
        });

        preencherTopo(ativa.nome);
        if (document.body.classList.contains('ui3')) {
            renderUi3(perfil, ativa, outras);
            return;
        }
        ...
        wrap.id = 'escolaSwitcherSidebar';
        wrap.style.cssText =
            'margin:10px 12px 4px;padding:10px 12px;border-radius:10px;' +
        ...
        atual.innerHTML =
            ...
            '</span>' +
            (outras.length
                ? '<i class="bi bi-chevron-down" style="font-size:12px;color:var(--text-tertiary,#a1a1aa);"></i>'
                : '');
        atual.querySelector('#escolaAtivaNome').textContent = ativa.nome;
        wrap.appendChild(atual);
        ...
        if (outras.length) {
            var lista = document.createElement('div');
            lista.style.cssText =
                'display:none;margin-top:8px;border-top:1px solid rgba(255,255,255,.08);padding-top:8px;';
            ...
            outras.forEach(function (e) {
                ...
                btn.innerHTML =
                    '<i class="bi bi-arrow-left-right" style="font-size:11px;color:var(--text-tertiary,#a1a1aa);"></i>';
                btn.appendChild(document.createTextNode(e.nome));
    ...
    function iniciar() {
        fetch(API_BASE + '/escolas/minhas', { credentials: 'include' })
            ...
            .catch(function () {
                /* sem switcher — sidebar segue normal */
            });
    }
