...
(function () {
    ...
    // API do cartão "Instalar" dos painéis (js/ui-painel.js). O mesmo
    // `deferredPrompt` da barra: o navegador só entrega um por página.
    window.PwaInstall = {
        instalado: isStandalone,
        disponivel: function () {
            return !!deferredPrompt || (isIOS() && !isStandalone());
        },
        instalar: function () {
            if (deferredPrompt) {
                var pedido = deferredPrompt;
                deferredPrompt = null;
                pedido.prompt();
                return pedido.userChoice
                    .then(function (choice) {
                        var aceito = !!(choice && choice.outcome === 'accepted');
                        var bar = document.getElementById('pwa-install-bar');
                        if (aceito && bar) bar.remove();
                        return aceito;
                    })
                    .catch(function () {
                        return false;
