...
const AnnouncementFeed: React.FC = () => {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: carrega o feed só na montagem
  useEffect(() => {
    loadFeed();
  ...
  if (loading && comunicados.length === 0) {
    return (
      ...
      >
        <RefreshCw
          style={{ animation: 'spin 1s linear infinite', color: 'var(--success-text)' }}
          size={32}
        />
        <p style={{ color: 'var(--text-tertiary)', fontSize: '0.875rem' }}>
          Buscando comunicados importantes...
        </p>
      </div>
    );
  }
    ...
    <div style={{ maxWidth: '768px', margin: '0 auto', padding: '0 16px 80px' }}>
      ...
      >
        ...
        <button
          ...
          onMouseLeave={(e) => {
            (e.currentTarget as HTMLButtonElement).style.background = 'rgba(var(--tint-rgb), 0.05)';
      ...
      {error ? (
        ...
        >
          ...
          <p style={{ color: 'var(--text-tertiary)', fontSize: '0.875rem', margin: 0 }}>
            Você está em dia com todos os avisos da escola.
          </p>
        </div>
      ) : (
