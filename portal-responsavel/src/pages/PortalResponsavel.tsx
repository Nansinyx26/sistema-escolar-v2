...
import type { Student, Grade, Attendance, Notification, GmailUser, AuthUser } from '../types';
...
function toGmailUser(u: AuthUser, googleProfile?: GmailUser | null): GmailUser {
  return {
    email: u.email,
    name: u.nome,
    // foto manual (GridFS) tem prioridade sobre Google; fallback para Google se não houver
...
const PortalResponsavel: React.FC = () => {
  ...
  const [currentTab, setCurrentTab] = useState<'dashboard' | 'linking' | 'profile' | 'bi'>('dashboard');
                          ...
                          <div className={styles.comunicadosSideCol}>
                            {activeStudent && (
                              <FichaAluno
                                ...
                                onUpdate={(partial) => {
                                  setStudents((prev: Student[]) => prev.map(s => s.id === activeStudent.id ? { ...s, ...partial } : s));
                                }}
                              />
                            )}
                          </div>
